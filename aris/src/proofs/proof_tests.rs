...
macro_rules! enumerate_subproofless_tests {
    ($x:ty, $y:ident) => {
        generate_tests! { $x, $y;
            ...
            test_tautcon, test_empty_rule, test_modus_tollens,
        }
    }
}
...
pub fn demo_proof_1<P: Proof>() -> P where P: PartialEq+std::fmt::Debug, PJRef<P>: PartialEq+std::fmt::Debug, P::SubproofReference: PartialEq+std::fmt::Debug {
    ...
    if let Some(sub) = prf.lookup_subproof(&r4) { let _: P::Subproof = sub; println!("lookup4 good"); } else { println!("lookup4 bad"); }
...
pub fn test_impintro<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
    let r1 = prf.add_premise(p("A"));
    let r2 = prf.add_premise(p("B"));
...
pub fn test_biconintro<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
...
pub fn test_equivintro<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
    let r1 = prf.add_premise(p("P -> Q"));
    let r2 = prf.add_premise(p("Q -> R"));
...
pub fn test_forallelim<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
...
pub fn test_forallintro<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug, PJRef<P>: Debug, P::SubproofReference: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
    let r1 = prf.add_premise(p("forall x, p(x)"));
...
pub fn test_existsintro<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug, PJRef<P>: Debug, P::SubproofReference: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
...
pub fn test_existselim<P: Proof+Debug>() -> (P, Vec<PJRef<P>>, Vec<PJRef<P>>) where P::Subproof: Debug, PJRef<P>: Debug, P::SubproofReference: Debug {
    use parser::parse_unwrap as p; use self::coproduct_inject as i;
    let mut prf = P::new();
