...
pub enum Line<T, U> {
    Direct(
        T,
        Justification<Expr, Coproduct<LineDep, Coproduct<LineDep, CNil>>, SubproofDep>,
    ),
...
impl<T: Clone, U: Clone> TreeSubproof<T, U> {
    fn count_lines(&self) -> usize {
        ...
        prf.0.premises.len()
            + prf
                ...
                .iter()
                .map(|line| {
                    if let Line::Subproof((SubproofSize(n), _), _) = line {
                        *n
                    } else {
                        1
                    }
    ...
    fn lookup_line(
        &self,
        i: usize,
    ) -> Option<Coprod!(Expr, Justification<Expr, Coprod!(LineDep, LineDep), SubproofDep>)> {
        ...
        for line in self.lines.iter() {
            match line {
                Line::Direct(_, just) => {
                    if j == i {
                        return Some(Coproduct::inject(just.clone()));
                    } else {
                        j += 1;
                    }
                }
                Line::Subproof(_, sub) => {
                    if let Some(x) = sub.lookup_line(i - j) {
                        return Some(x);
                    } else {
                        j += sub.count_lines();
                    }
...
impl<T: Clone + Default, U: Clone + Default> Proof for TreeProof<T, U> {
    ...
    fn lookup_premise(&self, LineDep(line): &Self::PremiseReference) -> Option<Expr> {
        self.0
            .lookup_line(line - 1)
            .and_then(|x| x.fold(hlist![|e| Some(e), |_| None]))
    }
    fn lookup_step(
        &self,
        LineDep(line): &Self::JustificationReference,
    ) -> Option<Justification<Expr, PJRef<Self>, Self::SubproofReference>> {
        self.0
            .lookup_line(line - 1)
            .and_then(|x| x.fold(hlist![|_| None, |j| Some(j)]))
    }
    fn lookup_subproof(
        &self,
        SubproofDep { .. }: &Self::SubproofReference,
    ) -> Option<Self::Subproof> {
        None // TODO: implement
    }
    ...
    fn add_step(
        ...
        self.0.lines.push(Line::Direct(Default::default(), just));
        let i = self.0.count_lines();
        LineDep(i)
    }
    ...
    fn add_step_relative(
        &mut self,
        _: Justification<Expr, PJRef<Self>, Self::SubproofReference>,
        _: &JSRef<Self>,
        _: bool,
    ) -> Self::JustificationReference {
        unimplemented!()
...
impl<A, B> TreeProof<A, B> {
    pub fn bimap<C, D, F: FnMut(A) -> C, G: FnMut(B) -> D>(
        self,
        f: &mut F,
        g: &mut G,
    ) -> TreeProof<C, D> {
    ...
    pub fn bimap<C, D, F: FnMut(A) -> C, G: FnMut(B) -> D>(
        ...
        TreeSubproof {
            premises: self
                .premises
                .drain(..)
                .map(|(data, premise)| (f(data), premise))
                .collect(),
...
pub enum PremiseOrLine<T> {
    Premise(T, Expr),
    Line(
        T,
        Justification<Expr, Coprod!(LineDep, LineDep), SubproofDep>,
    ),
}
...
pub fn decorate_line_and_indent<T, U>(prf: TreeProof<T, U>) -> TreeProof<(LineAndIndent, T), U> {
    fn aux<T, U>(
        mut prf: TreeSubproof<T, U>,
        li: &mut LineAndIndent,
    ) -> TreeSubproof<(LineAndIndent, T), U> {
        let mut result = TreeSubproof {
            premises: vec![],
            lines: vec![],
        };
        ...
        for line in prf.lines.drain(..) {
            match line {
                Line::Direct(data, just) => {
                    result.lines.push(Line::Direct((li.clone(), data), just));
                    li.line += 1;
                }
                Line::Subproof(data, sub) => {
                    li.indent += 1;
                    result.lines.push(Line::Subproof(data, aux(sub, li)));
                    li.indent -= 1;
                }
            }
        }
...
pub fn decorate_subproof_sizes<T, U>(prf: TreeProof<T, U>) -> TreeProof<T, (SubproofSize, U)> {
    fn aux<T, U>(mut prf: TreeSubproof<T, U>) -> (usize, TreeSubproof<T, (SubproofSize, U)>) {
        ...
        let mut result = TreeSubproof {
            premises: prf.premises,
            lines: vec![],
        };
        for line in prf.lines.drain(..) {
            match line {
                Line::Direct(data, just) => {
                    result.lines.push(Line::Direct(data, just));
                    size += 1;
                }
                Line::Subproof(data, sub) => {
                    let (sz, sp) = aux(sub);
                    ...
                    result
                        .lines
                        .push(Line::Subproof((SubproofSize(sz), data), sp));
                }
            }
        }
        (size, result)
...
pub fn lookup_by_decoration<T: Clone, F: Fn(&T) -> bool, U>(
    prf: &TreeSubproof<T, U>,
    f: &F,
) -> Option<PremiseOrLine<T>> {
...
pub fn check_rule_at_line(
    prf: &TreeProof<LineAndIndent, ()>,
    i: usize,
) -> Result<(), ProofCheckError<Coprod!(LineDep, LineDep), SubproofDep>> {
    if let Some(pol) = lookup_by_decoration(&prf.0, &|li| li.line == i) {
        ...
        Err(ProofCheckError::LineDoesNotExist(Coproduct::Inl(LineDep(
            i,
        ))))
    }
}
...
fn check_rule(
    prf: &TreeProof<LineAndIndent, ()>,
    li: LineAndIndent,
    Justification(expr, rule, deps, sdeps): Justification<
        Expr,
        Coprod!(LineDep, LineDep),
        SubproofDep,
    >,
) -> Result<(), ProofCheckError<Coprod!(LineDep, LineDep), SubproofDep>> {
    use ProofCheckError::*;
    for x in deps.iter() {
        let i = x.clone().fold(hlist![|LineDep(i)| i, |LineDep(i)| i]);
        if i >= li.line {
            return Err(ReferencesLaterLine(
                Coproduct::Inr(Coproduct::Inl(LineDep(li.line))),
                Coproduct::Inl(x.clone()),
            ));
        }
    }
    for &SubproofDep { start, end } in sdeps.iter() {
        ...
        if end >= li.line {
            return Err(ReferencesLaterLine(
                Coproduct::Inr(Coproduct::Inl(LineDep(li.line))),
                Coproduct::inject(SubproofDep { start, end }),
            ));
        }
    }
    rule.check(
        &prf.clone().bimap(&mut |_| (), &mut |_| ()),
        expr,
        deps,
        sdeps,
    )
