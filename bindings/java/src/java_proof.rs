...
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_toString(
    env: JNIEnv,
    obj: JObject,
) -> jstring {
    with_thrown_errors(&env, |env| {
        let ptr: jni::sys::jlong = env.get_field(obj, "pointerToRustHeap", "J")?.j()?;
        let prf: &LinedProof<PooledProof<Hlist![Expr]>> =
            unsafe { &*(ptr as *mut LinedProof<PooledProof<Hlist![Expr]>>) };
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_createProof(
    env: JNIEnv,
    _: JObject,
) -> jobject {
    with_thrown_errors(&env, |env| {
        let prf = Box::into_raw(Box::new(LinedProof::<PooledProof<Hlist![Expr]>>::new()));
        let jprf = env.new_object(
            "edu/rpi/aris/proof/RustProof",
            "(J)V",
            &[JValue::from(prf as jni::sys::jlong)],
        );
        Ok(jprf?.into_inner())
    })
}
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_addLine(
    env: JNIEnv,
    this: JObject,
    index: jni::sys::jlong,
    is_assumption: jni::sys::jboolean,
    subproof_level: jni::sys::jlong,
) {
    with_thrown_errors(&env, |env| {
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_setExpressionString(
    env: JNIEnv,
    this: JObject,
    index: jni::sys::jlong,
    text: JObject,
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_moveCursor(
    env: JNIEnv,
    this: JObject,
    index: jni::sys::jlong,
) {
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_fromXml(
    env: JNIEnv,
    _: JObject,
    jxml: JString,
) -> jobject {
    with_thrown_errors(&env, |env| {
        ...
        println!("{:?}", xml);
        if let Ok((prf, _)) = aris::proofs::xml_interop::proof_from_xml::<
            PooledProof<Hlist![Expr]>,
            _,
        >(xml.as_bytes())
        {
            println!("{}", prf);
            let prf = Box::into_raw(Box::new(
                LinedProof::<PooledProof<Hlist![Expr]>>::from_proof(prf),
            ));
            let jprf = env.new_object(
                "edu/rpi/aris/proof/RustProof",
                "(J)V",
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern "system" fn Java_edu_rpi_aris_proof_RustProof_checkRuleAtLine(
    env: JNIEnv,
    this: JObject,
    linenum: jni::sys::jlong,
) -> jstring {
    with_thrown_errors(&env, |env| {
        let ptr: jni::sys::jlong = env.get_field(this, "pointerToRustHeap", "J")?.j()?;
        let self_: &mut LinedProof<PooledProof<Hlist![Expr]>> = unsafe { &mut *(ptr as *mut _) };
